    ...
    <div class="container">
        ...
        <form class="mt-5" method="post" action="{{ route('posts.store') }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control @error('content') is-invalid @enderror" id="content" rows="5" name="content" placeholder="Content" value="{{ old('title') }}"></textarea>
                @error('content')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <button type="submit" class="mt-2 bnt btn-primary">Submit</button>
        </form>
    </div>
